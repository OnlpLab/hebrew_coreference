...
class ConllReader:
    ...
    def single_conll(self, chunk, merge_subtoken):
...
"""
st.title("Paste a Conll document format here, and see the magic")
st.header("Please use Firefox if tags are upsidedown")
user_input = st.text_area("", default_conll, height=500)

conll = ConllReader()
doc = conll.single_conll(user_input, merge_subtoken=False)
