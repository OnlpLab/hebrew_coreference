...
# Run SOTA tokenization evaluation for lingmess-coref
log "==== Starting SOTA tokenization evaluation for lingmess-coref ===="
for SEED in "${SEEDS[@]}"; do
  OUTDIR=$LINGMESS_OUT/$(basename $MODEL_NAME_OR_PATH)_seed${SEED}_sota_tokenized_eval
  # Use the trained model path from the regular evaluation
  ...
  if [ -d "$TRAINED_MODEL_PATH" ]; then
    ...
    log "[ERROR] [lingmess-coref] Seed $SEED: Trained model not found at $TRAINED_MODEL_PATH. Skipping SOTA tokenization evaluation."
  fi
done
