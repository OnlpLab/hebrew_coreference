...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print("Usage: python evaluate.py <input_json> <output_dir>")
        sys.exit(1)

    input_json = sys.argv[1]
    output_dir = sys.argv[2]

    # Try to load as single JSON first, then as JSONLines if that fails
    try:
        with open(input_json, "r", encoding="utf-8") as f:
            doc_predictions = json.load(f)
    except json.JSONDecodeError:
        # If single JSON fails, try JSONLines format
        doc_predictions = []
        with open(input_json, "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
                if line:  # Skip empty lines
                    doc_predictions.append(json.loads(line))

    os.makedirs(output_dir, exist_ok=True)
    get_evaluations(doc_predictions, output_dir)
