...
class LingMessCoref(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.antecedent_s2s_all_weights = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size, self.ffnn_size)))
        self.antecedent_e2e_all_weights = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size, self.ffnn_size)))
        self.antecedent_s2e_all_weights = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size, self.ffnn_size)))
        self.antecedent_e2s_all_weights = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size, self.ffnn_size)))

        self.antecedent_s2s_all_biases = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size)))
        self.antecedent_e2e_all_biases = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size)))
        self.antecedent_s2e_all_biases = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size)))
        self.antecedent_e2s_all_biases = nn.Parameter(torch.zeros((self.num_cats, self.ffnn_size)))

        self.reset_parameters()
        self.init_weights()
        print("Finish init LingMessCoref")
    ...
    def _get_clusters_labels(self, span_starts, span_ends, all_clusters):
        ...
        new_cluster_labels = torch.tensor(new_cluster_labels, device=self.device, dtype=torch.float32)
    ...
    def _get_categories_labels(self, tokens, subtoken_map, new_token_map, span_starts, span_ends):
        ...
        for b, (starts, ends) in enumerate(zip(span_starts.cpu().tolist(), span_ends.cpu().tolist())):
            ...
            for start, end in zip(starts, ends):
                ...
                span = {tokens[b][idx].lower().strip("_") for idx in token_indices if idx is not None}
                span = {i.strip("_")  for i in span}
        ...
        categories_labels = torch.tensor(categories_labels, device=self.device, dtype=torch.float32)
