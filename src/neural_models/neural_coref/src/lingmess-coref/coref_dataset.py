...
def create(tokenizer, train_file=None, dev_file=None, test_file=None, cache_dir='cache'):
    ...
    str.encode(str(tuple((k, v) for k, v in dataset_files.items())))
    cache_key = hashlib.md5(str.encode(str(tuple((k, v) for k, v in dataset_files.items()))) + str.encode(tokenizer.name_or_path)).hexdigest()
