...
def output_evaluation_metrics(metrics_dict, output_dir, prefix):
    ...
    muc_p, muc_r,muc_f1 =  metrics_dict['coref'].get_muc()
    bcubed_p, bcubed_r,bcubed_f1 =  metrics_dict['coref'].get_bcubed()
    ceafe_p, ceafe_r,ceafe_f1 =  metrics_dict['coref'].get_ceafe()
    results = {
        'eval_loss': loss,
...
def write_prediction_to_jsonlines(args, doc_to_prediction, doc_to_tokens, doc_to_subtoken_map, doc_to_new_word_map):
    ...
    with open(output_eval_file, "w") as writer:
        for doc in docs:
            doc_key = doc['doc_key']
            if doc_key not in doc_to_prediction:
                print("Check if there is an issue with the cache.")
                exit(1)
