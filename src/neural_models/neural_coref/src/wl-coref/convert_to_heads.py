...
DATA_DIR = "data/hebrew/wl_coref_docs"
FILENAME = "{}{}.hebrew.jsonlines"
...
class DynamicDepth():
    ...
    def get_parse_depths(self, heads, start, end):
        ...
        depths = [self._get_depth_recursive(h, 0) for h in range(len(self.relative_heads))]
        return depths

    @lru_cache(maxsize=None)
    def _get_depth_recursive(self, index, recur):
        ...
        if self.relative_heads[index] >= len(self.relative_heads) or self.relative_heads[index] < 0:
            return 0
        elif index == self.relative_heads[index]:
            return len(self.relative_heads)
        elif recur == 10:
            return 5
        return self._get_depth_recursive(self.relative_heads[index], recur+1) + 1
...
def get_head(mention: Tuple[int, int], doc: dict) -> int:
    ...
    cc_indexes = [i for i in range(end - start) if doc['pos'][i+start] == 'CCONJ' and depth[i] < depth_limit]
    if cc_indexes:
