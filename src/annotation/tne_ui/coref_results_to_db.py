...
def main():
    ...
    with con:
        cur = con.cursor()
        # DROP TABLE
        cur.execute("DROP TABLE IF EXISTS tne_coref_data")

        # CREATE TABLE
        cur.execute(
            "CREATE TABLE IF NOT EXISTS tne_coref_data(id INTEGER PRIMARY KEY AUTOINCREMENT, hit_id INTEGER, annotation_id INTEGER, clusters TEXT, states TEXT, annotator_id TEXT)")
    ...
    with open(in_file, encoding="utf8") as f:
        ...
        for ind, line in enumerate(data):
            ...
            hit_id = doc["hit_id"]
            cur.execute('select * from tne_coref_data where hit_id =' + str(hit_id))
            ...
            try:
                ...
                annotator_id = None
            cur.execute(
                "INSERT INTO tne_coref_data (hit_id, annotation_id, clusters, states, annotator_id) VALUES (?,?,?,?,?)",
                (hit_id, annotation_id, str(clusters), json.dumps(states), annotator_id))
            con.commit()
