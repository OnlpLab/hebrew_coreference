...
class Annotation:
    ...
    start: int
    stop: int
    id: Optional[str] = field(hash=False, compare=False, default=None)

    def fix_suffix_sofiot(self, text):
    ...
    def __eq__(self, other):
        """
        ONLY supports NP chunk (ignores label)
        :param other:
        :return:
        """
        return self.fix_suffix_sofiot(self.text) == self.fix_suffix_sofiot(other.text)

    def __hash__(self):
        return hash(self.fix_suffix_sofiot(self.text))
...
class Writer:
    DEFAULT_HEADER = "#FORMAT=WebAnno TSV 3.3\n" \
    ...
    def write(self, sentence: AnnotatedSentence):
        ...
        for _, group in groupby(sorted(
            ...
            schema = "|".join([f"*->{label_id}-1" for label_id, _ in annotations.items()]) \
